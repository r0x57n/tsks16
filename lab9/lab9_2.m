...
%% Noise
%delta = 0.0001; % original
delta = 0.00001; % max when e1 != e2
%delta = 0.00005; % max when e1 = e2

%% Quantization
B = 2^3;
% B = 2^3 works well for both
% x1 needs the largest B in order to be received without symbol errors
...
% be bad to phase (IQM/CFO/PO/NOISE)
y_rx_m = 0:length(y_rx) - 1;
Th = 1/(M*fs);

if NOISE
    e1 = randn(1, length(y_rx_m));
    e2 = randn(1, length(y_rx_m));
    phi1 = zeros(1, length(y_rx_m));
    phi2 = zeros(1, length(y_rx_m));

    phi1(1) = delta*e1(1);
    phi2(1) = delta*e2(1);
    for n = 2:length(y_rx_m)
...
if QUANT
    ...
    % Quant
    Quant = 2^(-(B-1));
    y_I_q = Quant*(floor((y_I_norm*(1-1e-10))/Quant) + 0.5);
    y_Q_q = Quant*(floor((y_Q_norm*(1-1e-10))/Quant) + 0.5);
    %Quant = 2^(-B);
    %y_I_q = Quant*(floor(y_I_norm/Q) + 0.5);
    %y_Q_q = Quant*(floor(y_Q_norm/Q) + 0.5);
